...
import { useAuth, Submission } from "@/lib/AuthContext";
      ...
      <div className="grid grid-cols-1 gap-6">
        {related.map((sub: any) => (
          <div
            key={sub.id}
            onClick={() => {
              if (sub.type === "video" && sub.link) {
                setActiveVideoUrl(sub.link);
                setActiveVideoTitle(sub.title);
                const creator = users.find(u => u.email.toLowerCase() === sub.userEmail.toLowerCase() || u.name.toLowerCase() === sub.userEmail.toLowerCase());
                setActiveVideoCreator(creator ? `@${creator.username}` : (sub.userEmail || "Creator"));
                setIsVideoOpen(true);
              } else if (sub.link) {
          ...
          >
            ...
            <div className="flex-1 flex flex-col justify-between py-1">
              ...
              <div className="inline-flex items-center gap-2 text-xs font-bold text-green-500 hover:text-green-400 uppercase tracking-widest transition-colors font-sans w-fit">
                {sub.type === "video" ? "Watch Vlog" : sub.link ? "Open Link" : "Read Full Story"}
